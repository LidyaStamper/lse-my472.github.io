...
#Create vector of titles - from all nodes only select title 
...
test_text <- test_url %>%
  read_html() %>%
  #Reads html - look at webpage (not RSS feed)
  html_nodes(css = "p") %>% 
  #Extract all nodes with paragraphs
  html_text() %>%
  #Transform into text
  paste(collapse = "\n")
...
#Sec = 2 (another variable, can override)
...
#Applies the function we made to every article in data_guardian articles (10)

Ask about difference between sapply and loop
